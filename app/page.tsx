...
export default function Home() {
  ...
  const countdown = useMemo(() => Math.max(0, Math.ceil((WEDDING.date.getTime() - now) / 86400000)), [now]);
  ...
  const addCalendar = () => {
    const calendar = ["BEGIN:VCALENDAR", "VERSION:2.0", "BEGIN:VEVENT", "DTSTART:20260815T080000Z", "DTEND:20260815T110000Z", "SUMMARY:이재린 조미연 웨딩 디너", `LOCATION:${WEDDING.venue} ${WEDDING.address}`, "END:VEVENT", "END:VCALENDAR"].join("\r\n");
    const url = URL.createObjectURL(new Blob([calendar], { type: "text/calendar;charset=utf-8" }));
    const link = document.createElement("a"); link.href = url; link.download = "이재린-조미연-웨딩디너.ics"; link.click(); URL.revokeObjectURL(url);
  };
  ...
  return <main className="shell">
    <audio ref={audioRef} src="/music.mp3" loop preload="metadata" onPlay={() => setMusicOn(true)} onPause={() => setMusicOn(false)} />
    {entryVisible && <EntryCover isLeaving={isEntering} onEnter={enterInvitation} />}
    <div className={`main-invitation ${hasEntered ? "is-open" : ""}`} aria-hidden={!hasEntered}>
    <section id="story" className="story panel">
      ...
      <div className="family-crop"><img src="/family-watercolor.png" alt="아기를 품에 안은 가족 수채화 일러스트" /></div>
    ...
    <section id="invitation" className="paper panel invitation">
      ...
      <div className="dress-code"><h3><span>DRESS CODE</span><i aria-hidden="true">·</i><b>RED POINT</b></h3><p>양복에 넥타이 대신 아기자기한 <em>붉은 꽃핀</em>이나,<br />부담없는 정장 대신 <em>붉은색</em>이 들어간 옷이나 아이템으로<br />센스 있게 포인트를 주고 와주세요.<br />함께 모여 사진을 찍을 때 더욱 예쁜 추억이 될 것 같습니다.</p></div>
    ...
    <section id="date" className="paper panel date">
      ...
      <div className="event-info"><p>2026. 08. 15 (토)</p><strong>오후 5시</strong><b>{WEDDING.venue}</b><button onClick={addCalendar}>▣&nbsp;&nbsp; 일정 저장하기</button><small>예식일까지 {countdown}일 남았습니다.</small></div>
    </section>
    ...
    <section id="gift" className="paper panel gift">
      ...
      <div className="bottom-flowers" aria-hidden="true">🌿　💌　🌼</div>
    </section>

    <section id="rsvp" className="paper panel rsvp">
      <Reveal><SectionTitle english="Message" korean="참석 의사" /></Reveal><div className="envelope" aria-hidden="true"><span>♥</span></div><p>소중한 걸음<br />진심으로 감사드립니다.<br />행복한 날에 함께 해주세요.</p><button className="primary-button" onClick={() => setRsvpOpen(true)}>✉&nbsp;&nbsp; 참석 여부 전하기</button>
    </section>

    <section id="thanks" className="thanks panel"><div className="thanks-copy"><h2>Thank you</h2><p>귀한 시간 내어 함께해 주셔서<br />진심으로 감사드립니다.</p><time dateTime="2026-08-15">2026.08.15</time></div><div className="thanks-family-frame"><img src="/thank-you-family-watercolor.png" alt="바닷가에 함께 앉은 가족 수채화 일러스트" /></div></section>
